...
%             pixel_annotation{iPixel,3} = acr;
    ...
    for pixel = 1:length(pixels_row)
        ...
        roi_annotation_fn{pixel} = baseFileName_slice;
...
% count number of ROIs per name-section and number of total pixels and total
% intensity (probably per mouse normalization necessary)
[uniq, uniq_idx, or_idx] = unique(join([roi_table_all.name, roi_table_all.filename]));
roi_count = accumarray(or_idx(:), 1, [], @sum);
pernameSection_roi_table = table(uniq, roi_table_all.name(uniq_idx), ...
            roi_table_all.filename(uniq_idx), ... 
            roi_table_all.acronym(uniq_idx), ... 
            roi_table_all.avIndex(uniq_idx), ... 
            roi_count, ...
            'VariableNames', {'nameSection', 'name', 'section', 'acronym', 'avIndex', ...
            'roi_count'});
[uniq, uniq_idx, or_idx] = unique(join([pixel_table_all.name, pixel_table_all.slice_name]));
signal_sum = accumarray(or_idx(:), pixel_table_all.signal_sum, [], @sum);
pixel_count = accumarray(or_idx(:), pixel_table_all.pixel_count, [], @sum);
signal_mean = signal_sum ./ pixel_count;
pernameSection_pixel_table = table(uniq, pixel_table_all.name(uniq_idx), ...
            pixel_table_all.slice_name(uniq_idx), ... 
            pixel_table_all.acronym(uniq_idx), ...
            pixel_table_all.avIndex(uniq_idx), ... 
            signal_sum, pixel_count, signal_mean, ...
            'VariableNames', {'nameSection', 'name', 'section', 'acronym', 'avIndex', ...
            'signal_sum', 'pixel_count', 'signal_mean'});
...
% merge collapsed ROI table with pixel_table to create pername table
pernameSection_table = outerjoin(pernameSection_pixel_table, pernameSection_roi_table, 'Keys', 'nameSection', 'RightVariables', 'roi_count');
pernameSection_table.roi_count(isnan(pernameSection_table.roi_count)) = 0;
pernameSection_table.roi_count_perpixel = pernameSection_table.roi_count ./ pernameSection_table.pixel_count;
[~, sort_idx] = sort(pernameSection_table.roi_count_perpixel, 'descend');
pernameSection_table = pernameSection_table(sort_idx, :);

% Save table with one entry for each structure
save(fullfile(processed_folder, 'pernameSection_table.mat'), 'pernameSection_table');
writetable(pernameSection_table, fullfile(processed_folder, 'pernameSection_table.csv'));

% % Save table with one entry for each structure
% save(fullfile(processed_folder, 'pernameSection_roi_table.mat'), 'pernameSection_roi_table');
% writetable(pernameSection_roi_table, fullfile(processed_folder, 'pernameSection_roi_table.csv'));
% save(fullfile(processed_folder, 'pernameSection_pixel_table.mat'), 'pernameSection_pixel_table');
% writetable(pernameSection_pixel_table, fullfile(processed_folder, 'pernameSection_pixel_table.csv'));
