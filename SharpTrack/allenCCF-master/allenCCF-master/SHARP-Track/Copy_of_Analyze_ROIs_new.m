...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
data_path = '/mnt/tosh/Projects/MEP/cell-counting/Immunostainings_TH_extra';
...
TH_masks_folder = fullfile(transformations_folder, '/TH_masks');
subject = tfa_split{end-2};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

processed_folder = fullfile(transformations_folder, '..');
maskfused_folder = fullfile(transformations_folder, 'maskfused_folder');
mkdir(maskfused_folder)

% Get a list of all files in the folder with the desired file name pattern and open them. 
filePattern_transform = fullfile(transformations_folder, '*.mat'); 
filePattern_slice = fullfile(transformations_folder, '*.tif'); 
filePattern_mask = fullfile(TH_masks_folder, '*.tif'); 

Files_transform = dir(filePattern_transform);
Files_slice = dir(filePattern_slice);
Files_mask = dir(filePattern_mask);

% go through masks
% go through regions

% go through pixels, if in mask count

pixel_table = cell(length(Files_transform), 1);
roi_table = cell(length(Files_transform), 1);
for k = 1 : length(Files_transform)
    % file location of transform, transformed image and csv file fro ROI creation
    baseFileName_transform = Files_transform(k).name;
    transform_location = fullfile(Files_transform(k).folder, baseFileName_transform);
    ...
    baseFileName_mask = Files_mask(k).name; 
    transformed_mask_location = fullfile(Files_mask(k).folder, baseFileName_mask);
...
%     end
    ...
    % load the mask image
    transformed_mask_image = imread(transformed_mask_location);
    transformed_mask_image = max(transformed_mask_image, [], 3); % take the maximum over RGB dim, assume only one channel contains all info
    transformed_mask_image = transformed_mask_image / max(transformed_mask_image(:)); % normalize transformed_mask_image
...
% 
    FIG = figure; 
    imshow(imfuse(transformed_mask_image, transformed_slice_image));
    title('transformed slice image, fused with TH mask')
    saveas(FIG, fullfile(maskfused_folder, ['maskfused_', 's', sprintf('%02d', k)]), 'tif')
%     
% %     FIG2 = figure; 
...
%     [pixels_row, pixels_column] = find(roi_array>0);
    ...
    in_mask = zeros(numel(transformed_slice_image), 1);
    ...
    for iX = 1:size(transformed_slice_image, 1)
        for iY = 1:size(transformed_slice_image, 2)
            ...
            in_mask(iPixel) = transformed_mask_image(iX, iY);
...
%             pixel_annotation{iPixel,3} = acr;
    ...
    all_pixel_table = table(pixel_annotation(:,2), pixel_annotation(:,3), ...
                ...
                signal_vec, in_mask, ...
                repmat({baseFileName_slice}, [length(signal_vec), 1]), ...
    'VariableNames', {'name', 'acronym', 'AP_location', 'DV_location', 'ML_location', 'avIndex', 'signal', 'in_mask', 'slice_name'});
    ...
    in_mask_sum = accumarray(or_idx(:), all_pixel_table.in_mask, [], @sum);
    ...
    'VariableNames', {'slice_name', 'name', 'acronym', 'avIndex', ...
        'pixel_count', 'signal_sum', 'in_mask_sum'});
...
in_mask_sum = accumarray(or_idx(:), pixel_table_all.in_mask_sum, [], @sum);
pixel_count = accumarray(or_idx(:), pixel_table_all.pixel_count, [], @sum);
signal_mean = signal_sum ./ pixel_count;
in_mask_percentage = (in_mask_sum ./ pixel_count) * 100;
pername_pixel_table = table(uniq, pixel_table_all.acronym(uniq_idx), ...
            ...
            in_mask_sum, in_mask_percentage, ...
            'VariableNames', {'name', 'acronym', 'avIndex', ...
            'signal_sum', 'pixel_count', 'signal_mean', ...
            'in_mask_sum', 'in_mask_percentage'});
[~, sort_idx] = sort(pername_pixel_table.in_mask_percentage, 'descend');
pername_pixel_table = pername_pixel_table(sort_idx, :);
...
save(fullfile(processed_folder, 'pername_pixel_table.mat'), 'pername_pixel_table');
writetable(pername_pixel_table, fullfile(processed_folder, 'pername_pixel_table.csv'));
